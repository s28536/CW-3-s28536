...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(
            e => grades.Where(s => e.Sal >= s.Losal && e.Sal <= s.Hisal),
            (e,s) => new {e.EName, s.Grade}).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        {
            DeptNo = g.Key, AvgSal = g.Average(s => s.Sal)
        }).ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > emps.Where(em => em.DeptNo == e.DeptNo)
            .Average(em => em.Sal)).Select(e => e.EName).ToList(); 
